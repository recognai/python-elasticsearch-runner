...
from elasticsearch_runner.runner import (
    ElasticsearchRunner,
    process_exists,
    parse_es_log_header,
)
...
class TestElasticsearchRunner(TestCase):
    ...
    def test_run(self):
        ...
        health_resp = requests.get(
            "http://localhost:%d/_cluster/health" % self.runner.es_state.port
        )
    ...
    def test_run_multiple(self):
        ...
        health_resp = requests.get(
            "http://localhost:%d/_cluster/health" % self.runner2.es_state.port
        )
        self.assertEqual(200, health_resp.status_code)
        health_data = json.loads(health_resp.text)
        self.assertEqual(health_data["status"], "green")
    ...
    def test_es_wrapper_call(self):
        ...
        self.assertEqual(
            runner._es_wrapper_call("nt"),
            [
                os.path.sep.join(
                    ["fakepath", runner.version_folder, "bin", "elasticsearch.bat"]
                )
            ],
        )
        self.assertEqual(
            runner._es_wrapper_call("posix"),
            [
                "/bin/sh",
                os.path.sep.join(
                    ["fakepath", runner.version_folder, "bin", "elasticsearch"]
                ),
            ],
        )
    ...
    def test_parse_log_header_esv2_format(self):
        ...
        testStream.write(
            u"[2015-10-08 11:21:09,025][INFO ][o.e.h.n.Netty4HttpServerTransport] [Hero] publish_address {127.0.0.1:9200}, bound_addresses {127.0.0.1:9200}, {[::1]:9200}\n"
        )
        testStream.write(
            u"[2015-10-08 11:04:15,784][INFO ][node                     ] [Hero] started\n"
        )
        testStream.seek(0)
    ...
    def test_parse_log_header_esv1_format(self):
        ...
        testStream.write(
            u"[2015-10-08 11:04:15,784][INFO ][o.e.h.n.Netty4HttpServerTransport] [Astronomer] bound_address {inet[/0:0:0:0:0:0:0:0:9200]}, publish_address {inet[/10.0.80.134:9200]}\n"
        )
        testStream.write(
            u"[2015-10-08 11:04:15,784][INFO ][node                     ] [Astronomer] started\n"
        )
        testStream.seek(0)
        server_pid, es_port = parse_es_log_header(testStream)
